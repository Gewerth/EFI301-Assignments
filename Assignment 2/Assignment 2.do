...
/*  Problem 6
	1.  Consider the following model of demand on a given airline route
		...
		a, Interpret the coefficient β1.
			if the fare increse by 100% then the  number of passengers increase/decrease  by B_1*100%.  		
			β1  is  the relation between fare and passengers. 
...
------------------------------------------------------------------------------
		If the fare increse by 100% then the  number of passengers increase(decrease) by -48,2%. */
...
/*	2. Our application here is an example of demand estimation in a many markets framework. What is a market in our application? 
	## The market is the airline market, a market in perfect competition where buyers buy seats from airlines.  In our application we aproximate it by a linear function at one point in time. ##*/
...
/*		b) What sign do you expect for β2?*/
		/* We expect that the cost to fly 10+h flights is larger than the cost to fly 2h flights.  Hence we expect β2>0 */

/*		c) What sign do you expect for the covariance between log(fare) and log(dist)? */
gen ldist = log(dist)
correlate lfare ldist, covariance
		/*After some googleing of airfares, we've seen that the variance in price increses the longer the travle is. This would indicate that the covarience is positive.*/
		
/*		d) What sign do you expect for the omitted variable bias? */
		/*since the denumerator is >0 by definition and the nominator is >0 and  β2>0 the OV-bias is positive */
...
------------------------------------------------------------------------------
*/
/* Yes, we where right, the OLS-bias had made the β_1 larger than it in fact is. 
	-.4815181 = OLS_bias + -.6128103, which emplies that ols-bias>0 */
...
*/ 
correlate ldist lfare, covariance
/*             |    ldist    lfare
-------------+------------------
       ldist |  .434984
       lfare |  .160316  .173921
...
di  .160316 / .173921 /* =.92177324*/

/* Thus, the expression (III) becomes  -.6128103 + .92177324 * .1424345 = -.48151799/
di -.6128103 + .92177324 * .1424345*/
...
/*	2. Fares for air travel are determined in a competitive market by the laws of supply and demand. Explain intuitively why this renders the exogeneity assumption (3) implausible.*/
		...
		/* Fares are in the market the result of suply and demand.  We can express the demanded quantity as a linear function alpa_0 - alpha_1*p and suplied quantity as a function eta_0 +eta_a*p  where p is price, variable_0 is amount produced and consumed when the price is 0, and var_1 is the increase(decrease) in quantity produced(demanded) if the price increased by 1. We know from economics that for every price only one possible number of products are exchanged. Thus eta_0 +eta_a*p = alpa_0 - alpha_1*p. To this model, we introduce a rapid change in demand, we call it u, thus  eta_0 +eta_a*p = alpa_0 - alpha_1*p +u. We have now spotted a feed-back loop, the demanded quantity is dependent on this chock i.e if there is a rapid change in price, the demand and suply change is dependent on the price. Thus we can learn somthing about the dependent variable from the independent ones.*/
...
/*		a) Explain in intuitive terms (drawing from your knowledge of economic theory) how bmktshr influences the supply side of the market.*/
	/*When an actor on the market has monopoly power, he earns price setting power, he does this through limiting suply. This maximizes his profit. Thus, large bmktshr would emply limited compitition and therfore suply more dependent on the profit function of the largest actor.*/
...
/*		b) Suppose that one carrier stops providing service on a route so that bmktshr for this route increases. What happens to the supply curve for this route? What happens to the ticket price? Is the price change informative about the demand shock?*/
	/* When bmktshr increase, the monopoly power increase, thereby the suply curve aproximates the proffit curve of the supliers. This will either raise ticket prices or keep them at the same price. This price change has informative powers on the demand chock. */
...
/*		b) Do you expect it to be satisfied? Explain.*/ 
	...
	/*Yes, we expect that the larger dominance an actor has on a market, the hifgher the prices will be, from the argument in 3.a. */
...
relevance assumption is not satisfied and in which we still have cov(bmtshr, log(fare))!= 0. Assume that passengers are willing to spend more money the longer the distance travelled. What would the joint distribution of log(dist) and bmtshr have to look like to ensure that that cov(bmtshr, log(fare)) < 0. */
...
/*		a) State the appropriate instrument exogeneity assumption.*/
		/*general form, E[U|Z_1, X_2, ... X_k]=0
		  in this case E[U|bmktshr, ldist]=0        */
/*		b) Do you expect it to be satisfied? Explain.*/
	/*Yes, since bmktshr has no explanetory power over the error-term, nor does distance.*/
	

/*	7. Instruments like market share that affect the supply side of the market are called "supply shifters". Supply shifters are often used to estimate demand parameters. Explain briefly in intuitive terms*/

/*		a) why a supply shifter is a relevant instrument. */
	/*Suply shifters move the suply curve verticaly, thus enableing us to find the component that moves in horizontal direction, we besically create the ceterus paribus effect by decomposing the vector of the suply function into two parts the supply shifter, and the ceteris paribus part*/
...
/*		b) Give an econometric explanation for the different estimates. */
		/* We don't take the ceteris paribus effect in the OLS-regresion, Since fare is endogenous to the error term, since mktshr determins pricing power, and therefore fares.*/

/*	9. Now we want to compute the IV estimate β_1ˆiv(ω0) by implementing the two-stage
...
regress lpassen lfare_hat ldist, robust 
/*------------------------------------------------------------------------------
             |               Robust
     lpassen | Coefficient  std. err.      t    P>|t|     [95% conf. interval]
-------------+----------------------------------------------------------------
   lfare_hat |  -3.307796    .658582    -5.02   0.000    -4.599957   -2.015634
       ldist |   1.135688   .2474185     4.59   0.000     .6502436    1.621132
       _cons |   15.49878   1.789444     8.66   0.000     11.98783    19.00974
------------------------------------------------------------------------------
 */

/*Wiho! they are the same!. */
